...
// Но эта функция даёт типизацию и помогает инструментам разработки (IDE, ESLint, TS) понять,
// что здесь описан компонент, а не просто объект.
...
const App = defineComponent({
  ...
  data() {
    return {
      ...
      // Всё, что определяет UI - состояние элементов форм, отображаемое содержимое - должно
      // определяться состоянием приложения
      filter: {
        date: 'all',
        participation: 'all',
        search: '',
  ...
  // Вычисляемые свойства позволяют иметь свойства компонента,
  // значение которых автоматически вычисляется на основе других реактивных свойств
  computed: {
    // Описываем функцию вычисления значения свойства
    filteredMeetups() {
...
// Возвращается экземпляр корневого компонента
